...
const ResearchDevelopment = () => {
    ...
    useEffect(() => {
        setProjects(magento.MAGENTO_SUCCESS_STORIES);
    }, []);

    const filterCategrory = (projectType) => {
        let resultFiltered = magento.MAGENTO_SUCCESS_STORIES.filter(
    ...
    return (
        <>
            ...
            <section className="section">
                <div className="max-w-7xl mx-auto px-5">
                    <div className="text-center pb-10">
                        <h2 className="section__title">{magento.offer.title}</h2>
                    </div>
                    <div className="grid md:grid-cols-3 gap-8">
                        {magento.MAGENTO_WHAT_WE_OFFERING.map((item, index) => (
                            <div key={index}>
                                <img
                                    loading="lazy"
                                    src={item.img}
            ...
            <section className="section">
                <div className="max-w-7xl mx-auto px-5">
                    <div className="text-center pb-10">
                        <h2 className="section__title">{magento.stories.title}</h2>
                        <div className="hidden lg:flex flex-wrap space-x-4 justify-center items-center my-8">
                            {magento.BUTTONS_ECOMMERCE &&
                                magento.BUTTONS_ECOMMERCE.map((button, index) => (
                                    <button
                                        key={button.id}
